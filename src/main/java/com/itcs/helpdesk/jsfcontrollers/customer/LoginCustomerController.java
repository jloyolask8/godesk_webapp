...
public class LoginCustomerController extends AbstractManagedBean<Cliente> implements Serializable {
    ...
    public String login() {
        ...
        if (caso != null) {
            ...
            casoController.prepareCasoFilterForInbox();
//            casoController.prepareCustomerViewCaso(caso);
            casoController.setSelected(caso);
            casoController.prepareDynamicCaseData(caso);
...
//                System.out.println("Se encontro caso:" + idCaso);
        ...
        if (casoController.getSelected() == null) {
            //No hay caso seleccionado.
            redirectToView("customer/login");
        }
...
//      HashMap<Long, String> casosMap = new HashMap<Long, String>();
        ...
        try {
            String html = "<html>"
                    ...
                    + "<br/></html>";
            MailClientFactory.getInstance(getUserSessionBean().getTenantId(),EnumAreas.DEFAULT_AREA.getArea().getIdArea())
                    .sendHTML(email, "Resumen de sus Casos", html, null);

...
//    }
    ...
    public void handleSmtpSslOrTlsChange() {
        if (getSelected() != null && !StringUtils.isEmpty(getSelected().getMailSmtpEncriptionType())) {
            if (getSelected().getMailSmtpEncriptionType().equalsIgnoreCase("STARTTLS")) {

                getSelected().setMailSmtpSslEnable(false);
                getSelected().setMailTransportTls(true);

            } else if (getSelected().getMailSmtpEncriptionType().equalsIgnoreCase("SSL")) {
                getSelected().setMailSmtpSslEnable(false);
                getSelected().setMailTransportTls(true);
            } else {
                getSelected().setMailSmtpSslEnable(false);
            ...
            getSelected().setMailSmtpEncriptionType("NONE");
            getSelected().setMailSmtpSslEnable(false);
            getSelected().setMailTransportTls(false);
        }
    ...
    public void handleMailStoreProtocolChange() {
        if (getSelected().getMailStoreProtocol() != null && getSelected().getMailStoreProtocol().endsWith("s")) {
            getSelected().setMailInboundSslEnabled(true);
        } else {
            getSelected().setMailInboundSslEnabled(false);
        }
    }
...
//        editActiveIndex = 0;
    ...
    public String create() {
        try {
            ...
            Log.createLogger(this.getClass().getName()).log(Level.SEVERE, "error al tratar de crear area", e);
        ...
        try {
            ...
            if (current.getEmailEnabled() != null && current.getEmailEnabled()) {
                ...
                try {
                    ...
                    MailClientFactory.createInstance(getJpaController().getSchema(), current);
                    HelpDeskScheluder.scheduleRevisarCorreo(getJpaController().getSchema(), current.getIdArea(), current.getEmailFrecuencia());
                ...
                try {
                    ...
                    final String downloadEmailJobId = DownloadEmailJob.formatJobId(getJpaController().getSchema(), current.getIdArea());

...
//    public ParseCotizacionAction(JPAServiceFacade jpaController, ManagerCasos managerCasos) {
...
//     protected abstract String executeBeforeMerge(Caso caso) throws ActionExecutionException;
    ...
    public void execute(final Caso caso, final String schema) throws ActionExecutionException {
        try {
            ...
            System.out.println("datos collected:" + datos);
            ...
            try {
                //check canal
                Canal portalInmob = em.find(Canal.class, getCanalId());
                if (portalInmob == null) {
                    portalInmob = new Canal(getCanalId());
                    portalInmob.setNombre(getCanalName());
                    em.persist(portalInmob);
                }
...
//            caso.setIdPrioridad(EnumPrioridad.MEDIA.getPrioridad());
                ...
                String idUser = this.getClass().getSimpleName();
                AuditLog audit = new AuditLog();
                audit.setIdUser(idUser);
                audit.setFecha(Calendar.getInstance().getTime());
                audit.setTabla(Caso.class.getSimpleName());
                audit.setNewValue("se parsea descripción del caso como " + caso.getTipoCaso().getNombre());

...
//        values.add(SALUDO_CLIENTE);
    ...
    public static String buildFinalText(String templateString, Caso caso, String schema) {
//        System.out.println("templateString:" + templateString);
        ...
        try {
            final String saludoH = ApplicationConfig.getInstance(schema).getSaludoClienteHombre();
            final String saludoM = ApplicationConfig.getInstance(schema).getSaludoClienteMujer();
            valuesMap.put(SALUDO_CLIENTE, (caso.getEmailCliente().getCliente().getSexo().equalsIgnoreCase("Hombre") ? saludoH : saludoM));
        } catch (Exception e) {
            valuesMap.put(SALUDO_CLIENTE, ApplicationConfig.getInstance(schema).getSaludoClienteUnknown());
        }

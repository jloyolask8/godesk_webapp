...
//    private ResourceBundle resourceBundle;
    ...
    private JPAServiceFacade getJpaController() {
        if (null == jpaController) {
            jpaController = new JPAServiceFacade(utx, emf, null);//TODO Change schema dynamic
...
//    }
    ...
    public WSResponse crearCasoCliente(@WebParam(name = "rut") final String rut,
        ...
        try {
            ...
            if (idProducto != null) {
                try {
                    Producto prod = getJpaController().find(Producto.class, idProducto);//.getProductoFindByIdProducto(idProducto);
...
//                        }
            ...
            int endIndex = caso.getDescripcionTxt() != null ? caso.getDescripcionTxt().length() : 0;
...
//            caso.setIdArea(EnumAreas.DEFAULT_AREA.getArea());//This is important to know what email received the ticket, but in WS there is no specific email.
            getManagerCasos().persistCaso(caso, getManagerCasos().createLogComment(caso, "se crea caso desde canal " + caso.getIdCanal().getNombre()));
